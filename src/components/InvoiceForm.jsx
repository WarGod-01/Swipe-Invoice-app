...
const InvoiceForm = () => {
  ...
  const products = useSelector(selectInvoiceList);
  ...
  const onCurrencyChange = async (selectedOption) => {
    ...
    const toCurrencyCode = currencyMap.get(currency);

    const currentCurrency = formData.currency;
    const currentCurrencyCode = currencyMap.get(currentCurrency);
    ...
    const updatedItems = await Promise.all(
      formData.items.map(async (item) => {
        ...
        return {
          ...item,
          itemPrice: convertedAmount.toFixed(2),
        };
      })
    );
    ...
    const updatedSubTotal = updatedItems.reduce(
      (acc, curr) =>
        acc + parseFloat(curr.itemPrice) * parseInt(curr.itemQuantity),
      0
    );

    const taxAmount = (updatedSubTotal * (formData.taxRate / 100)).toFixed(2);
        ...
        <Col md={4} lg={3}>
          <div className="sticky-top pt-md-3 pt-xl-4">
            ...
            <Form.Group className="mb-3">
              ...
              >
                <option value="$" valuetype="USD">
                  {" "}
                  (United States Dollar)
                </option>
                <option value="₹" valuetype="INR">
                  {" "}
                  (Indian Rupee)
                </option>
                <option value="£" valuetype="GBP">
                  {" "}
                  (British Pound Sterling)
                </option>
                <option value="¥" valuetype="JPY">
                  {" "}
                  (Japanese Yen)
                </option>
                <option value="$" valuetype="CAD">
                  {" "}
                  (Canadian Dollar)
                </option>
                <option value="$" valuetype="AUD">
                  {" "}
                  (Australian Dollar)
                </option>
                <option value="$" valuetype="SGD">
                  {" "}
                  (Singapore Dollar)
                </option>
                <option value="¥" valuetype="CNY">
                  {" "}
                  (Chinese Renminbi)
                </option>
                <option value="₿" valuetype="BTC">
                  {" "}
                  (Bitcoin)
                </option>
              </Form.Select>
